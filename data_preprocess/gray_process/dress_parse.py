...
def get_bottom_agnostic(im, im_parse, pose_data):
    ...
    # parse_head = parse_head * 255
    # kernel = np.ones((round(1), round(1)), np.uint8)
    # parse_head = cv2.dilate(parse_head, kernel, iterations=1)
    ...
    parse_bag = (parse_array == 16).astype(np.float32)
    ...
    agnostic_draw.rectangle((pointx - r * 20, (pointy + r * 5) - r * 35, pointx + r * 20, (pointy + r * 5) + r * 45), 'gray', 'gray')
    gray_draw.rectangle((pointx - r * 20, (pointy + r * 5) - r * 35, pointx + r * 20, (pointy + r * 5) + r * 45), 'gray', 'gray')
    ...
    for i in [9, 12]:
        ...
        agnostic_draw.rectangle((pointx - r * 10, pointy - r * 10, pointx + r * 10, pointy + r * 10), 'gray',
                                    'gray')
    ...
    # mask arms
    # agnostic_draw.line([tuple(pose_data[i]) for i in [2, 3]], 'gray', width=r * 12)
    # agnostic_draw.line([tuple(pose_data[i]) for i in [3, 4]], 'gray', width=r * 12)
    # agnostic_draw.line([tuple(pose_data[i]) for i in [2, 4]], 'gray', width=r * 12)
    # agnostic_draw.line([tuple(pose_data[i]) for i in [5, 6]], 'gray', width=r * 12)
    # agnostic_draw.line([tuple(pose_data[i]) for i in [6, 7]], 'gray', width=r * 12)
    # agnostic_draw.line([tuple(pose_data[i]) for i in [5, 7]], 'gray', width=r * 12)
    # agnostic_draw.line([tuple(pose_data[i]) for i in [2, 5]], 'gray', width=r * 12)
    #
    # gray_draw.line([tuple(pose_data[i]) for i in [2, 3]], 'gray', width=r * 12)
    # gray_draw.line([tuple(pose_data[i]) for i in [3, 4]], 'gray', width=r * 12)
    # gray_draw.line([tuple(pose_data[i]) for i in [2, 4]], 'gray', width=r * 12)
    # gray_draw.line([tuple(pose_data[i]) for i in [5, 6]], 'gray', width=r * 12)
    # gray_draw.line([tuple(pose_data[i]) for i in [6, 7]], 'gray', width=r * 12)
    # gray_draw.line([tuple(pose_data[i]) for i in [5, 7]], 'gray', width=r * 12)
    ...
    for i in [3, 4, 6, 7]:
        if (pose_data[i - 1, 0] == 0.0 and pose_data[i - 1, 1] == 0.0) or (pose_data[i, 0] == 0.0 and
            ...
            gray_draw.line([tuple(pose_data[j]) for j in [i - 1, i]], 'gray', width=r * 10)
        pointx, pointy = pose_data[i]
        # agnostic_draw.ellipse((pointx - r * 5, pointy - r * 5, pointx + r * 5, pointy + r * 10), 'gray', 'gray')
        # gray_draw.ellipse((pointx - r * 5, pointy - r * 5, pointx + r * 5, pointy + r * 10), 'gray', 'gray')
    ...
    agnostic.paste(im, None, Image.fromarray(np.uint8(parse_bag * 255), 'L'))
    # agnostic.paste(im, None, Image.fromarray(np.uint8(parse_fullbody * 255), 'L'))

    gray.paste(white, None, Image.fromarray(np.uint8(parse_hair * 255), 'L'))
    gray.paste(white, None, Image.fromarray(np.uint8(parse_head * 255), 'L'))
    # gray.paste(white, None, Image.fromarray(np.uint8(parse_arm * 255), 'L'))
    ...
    gray.paste(white, None, Image.fromarray(np.uint8(parse_bag * 255), 'L'))
    # gray.paste(white, None, Image.fromarray(np.uint8(parse_fullbody * 255), 'L'))
...
if __name__ == '__main__':
    imagelist = os.listdir('/home/vera/Dress_VITON/Dress_master_2501_test/dataset_0315/model/image')
    person_image_dir = '/home/vera/Dress_VITON/Dress_master_2501_test/dataset_0315/model/image/'
    parse_dir = '/home/vera/Dress_VITON/Dress_master_2501_test/dataset_0315/model/image-parse-v3/'
    openpose_img_dir = '/home/vera/Dress_VITON/Dress_master_2501_test/dataset_0315/model/openpose_img/'
    openpose_json_dir = '/home/vera/Dress_VITON/Dress_master_2501_test/dataset_0315/model/openpose_json/'

    despath = '/home/vera/Dress_VITON/Dress_master_2501_test/dataset_0315/model/agnostic-v3.2'
    des_gray_path = '/home/vera/Dress_VITON/Dress_master_2501_test/dataset_0315/model/gray'
